...
class Simulation(object):
    ...
    def __init__(self, pop_size, vacc_percentage, virus, initial_infected=1):
        ...
        # allow the logger to be the file name
        self.logger = Logger(self.file_name)  # file
    ...
    def _create_population(self):
        ...
        vaccinated = int(self.pop_size * vacc_percentage)
        ...
        for i in range(counter, infected + counter):
            counter += 1
    ...
    def _simulation_should_continue(self):
        ...
        for person in self.population:
            if person.is_vaccinated:
                vac_count.append(person)

        if len(self.population) == self.total_dead:
            return False
        elif len(vac_count) == len(self.population):
            return False
        else:
            return True
        ...
        while should_continue:
            ...
            for person in self.population:
                self.logger.log_infection_survival(
                    person, person.did_survive_infection())
    ...
    def interaction(self, person, random_person):
       ...
       # Check to see if the person is infected
        if random_person.is_vaccinated:
            ...
            deciscion = uniform(0, 1)
            if deciscion < self.virus.repro_rate:
...
def test_run():
    ...
    virus = Virus("HIV", 0.50, 0.70)
    # create new simulation object
    simulator = Simulation(1, 0.90, virus, 1)
    # run test simulation
    simulator.run()
